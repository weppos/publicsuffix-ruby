...
module PublicSuffix
  ...
  class << self
    ...
    def parse(name, list: List.default, default_rule: nil, ignore_private: false)
      what = normalize(name)
    ...
    # @param  [Boolean] ignore_private
    # @return [Boolean]
    def valid?(name, list: List.default, default_rule: nil, ignore_private: true)
      what = normalize(name)
